...
export const KPI_PRESETS: Record<BusinessProfile, Preset> = {
  ...
  local_services: {
    with_ga4: ['gbp_calls', 'leads_generated', 'gbp_website_clicks', 'reviews_new_total'],
    without_ga4: ['gbp_calls', 'leads_manual', 'gbp_website_clicks', 'reviews_new_total'],
  },
  ecommerce: {
    with_ga4: ['ecommerce_transactions', 'ecommerce_revenue', 'ecommerce_cpa', 'traffic_organic_estimate'],
    without_ga4: ['traffic_organic_estimate', 'traffic_branded', 'reviews_new_total', 'gsc_clicks'],
  },
  b2b_saas: {
    with_ga4: ['activations', 'cost_per_activation', 'traffic_organic_estimate', 'gsc_clicks'],
    without_ga4: ['traffic_organic_estimate', 'gsc_clicks', 'keywords_indexed_top10', 'leads_manual'],
  },
  b2b_services: {
    with_ga4: ['leads_generated', 'cost_per_lead', 'traffic_organic_estimate', 'gsc_clicks'],
    without_ga4: ['leads_manual', 'traffic_organic_estimate', 'gsc_clicks', 'keywords_indexed_top10'],
  },
  media: {
    with_ga4: ['traffic_organic_estimate', 'gsc_clicks', 'engagement_total', 'keywords_indexed_top10'],
    without_ga4: ['traffic_organic_estimate', 'gsc_clicks', 'engagement_total', 'keywords_indexed_top10'],
  },
  freelance_personal: {
    with_ga4: ['traffic_organic_estimate', 'traffic_branded', 'gsc_clicks', 'keywords_indexed_top10'],
    without_ga4: ['traffic_organic_estimate', 'traffic_branded', 'gsc_clicks', 'keywords_indexed_top10'],
  },
  unknown: {
    // Fallback when we couldn't detect a profile — show generic business signals
    with_ga4: ['traffic_organic_estimate', 'leads_manual', 'reviews_new_total', 'gsc_clicks'],
